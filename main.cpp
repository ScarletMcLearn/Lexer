...
public:
    ...
    bool W(char c) {return(_W.find(c) == _W.end()?false:true);}
    string Key(string c)    {return(_KEY.find(c) == _KEY.end()?"NULL":_KEY[c]);}
    string Punc(string c)   {return(_PUNC.find(c) == _PUNC.end()?"NULL":_PUNC[c]);}
    string Op(string c)     {return(_OP.find(c) == _OP.end()?"NULL":_OP[c]);}
